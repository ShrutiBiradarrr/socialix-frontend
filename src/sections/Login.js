  ...
  const handleLogin = async () => {
    ...
    try {
      const { data } = await axios.post(
        "https://socialix-social-media-backend.vercel.app/api/auth/login",
        {
          userName,
          password,
        }
      ...
      <div className="h-screen flex auth-page justify-center align-middle items-center">
        <div className="login-box bg-white shadow-lg rounded border-l-4 border-blue-300 md:flex p-5 md:w-1/2 w-[90%]">
          ...
          <div className="inputs md:w-[60%] ml-4 p-2">
            ...
            <button
              onClick={handleLogin}
              className="p-1 my-8 rounded-md bg-blue-400 font-bold w-full text-white hover:bg-blue-600 shadow "
            >
              Login
            </button>
            <Link to="/signup">
